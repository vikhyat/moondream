...
class TextModel:
    def __init__(self, model_path: str = "model") -> None:
        ...
        self.model = load_checkpoint_and_dispatch(self.model, f"{model_path}/text_model.pt", device_map={"": self.device.type},)
        self.text_emb = self.model.get_input_embeddings().to(self.device)

    def input_embeds(self, prompt, image_embeds):
        
        def _tokenize(txt):
            return self.tokenizer(txt, return_tensors="pt", add_special_tokens=False).input_ids.to(self.model.device)

        # Add BOS token
        embeds = []
        embeds.append(self.text_emb((torch.tensor([[self.tokenizer.bos_token_id]], device=self.model.device))))
    ...
    def generate(self, image_embeds, prompt, eos_text="Human:", max_new_tokens=128, **kwargs):
        ...
        with torch.no_grad():
            ...
            output_ids = self.model.generate(inputs_embeds=inputs_embeds, **generate_config)

        return self.tokenizer.batch_decode(output_ids, skip_special_tokens=True)

    def answer_question(self, image_embeds, question, **kwargs):
        prompt = f"<image>\n\nQuestion: {question}\n\nAnswer:"
        answer = self.generate(image_embeds, prompt, eos_text="<END>", max_new_tokens=128, **kwargs,)[0]
