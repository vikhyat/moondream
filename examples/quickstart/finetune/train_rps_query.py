...
Dataset: moondream/rps-finetune

Requires:
    pip install datasets pillow

To run with a local moondream-python checkout:
    PYTHONPATH=/path/to/moondream-python python train_rps_query.py
...
QUESTION = (
    "Is this rock, paper, or scissors? "
    "Respond with one of: rock, paper, scissors only."
)
...
def evaluate(ft, examples):
    ...
    for example in examples:
        ...
        correct += normalize_class(answer) == example["class"]
    return correct / len(examples)


def main():
    train_examples = load_examples("train", shuffle=True)
    eval_examples = load_examples("valid", limit=EVAL_LIMIT, shuffle=True)
    ...
    for _, (example, response) in zip(
        ...
        rewards = [
            float(normalize_class(r["output"]["answer"]) == example["class"])
